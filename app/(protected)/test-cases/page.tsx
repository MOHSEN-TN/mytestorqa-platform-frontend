...
export default function TestCasesPage() {
  ...
  const { loading: loadingTestCases, creating, updating, deleting, error: testCaseError } = useAppSelector((s) => s.testCases);
...
useEffect(() => {
  ...
  const paginated = testCaseList.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  const handleCreateTestCase = async (e: React.FormEvent) => {
    ...
    if (createTestCase.fulfilled.match(r)) { setIsCreateOpen(false); resetCreate(); dispatch(fetchTestCases({ suiteId: selectedSuite.id, status: statusFilter, priority: priorityFilter })); }
  };
  const handleDuplicate = async (id: string) => {
    if (!selectedSuite?.id) return;
    const r = await dispatch(duplicateTestCase({ suiteId: selectedSuite.id, testCaseId: id }));
    if (duplicateTestCase.fulfilled.match(r)) dispatch(fetchTestCases({ suiteId: selectedSuite.id, status: statusFilter, priority: priorityFilter }));
  };
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    if (updateTestCase.fulfilled.match(r)) { closeEditModal(); dispatch(fetchTestCases({ suiteId: selectedSuite.id, status: statusFilter, priority: priorityFilter })); }
  };
  const handleDelete = async (id: string) => {
    ...
    if (deleteTestCase.fulfilled.match(r)) dispatch(fetchTestCases({ suiteId: selectedSuite.id, status: statusFilter, priority: priorityFilter }));
  };
          ...
          <section className={`relative min-w-0 rounded-xl border border-gray-100 bg-white shadow-sm ${casesColCls}`}>
            ...
            <div className="px-5 py-4 border-b border-gray-50 space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <select value={itemsPerPage} onChange={e => setItemsPerPage(Number(e.target.value))} disabled={!selectedSuite}
                  className="rounded-lg border border-gray-200 px-2 py-1.5 text-sm text-gray-600 disabled:opacity-40 focus:outline-none">
              ...
              {!selectedSuite ? (
                ...
                <>
                  ...
                  <div className="flex items-center justify-between pt-2">
                    <span className="text-xs text-gray-400">{testCaseList.length} résultat(s)</span>
